  ...
  <div class="d-flex flex-wrap align-center ga-2">
      ...
      <v-menu v-model="menuStart" :close-on-content-click="false" location="bottom start">
        <template v-slot:activator="{ props }">
          <v-btn v-bind="props" variant="outlined" density="default"
            ...
            style="width: 200px; border-color: rgba(0,0,0,0.12);" prepend-icon="mdi-calendar" flat>
            {{ startDate ? formatDate(startDate) : 'Επιλέξτε' }}
          </v-btn>
        </template>
        <v-card min-width="auto">
          <v-date-picker v-model="internalStartDate" @update:model-value="onStartDateSelected" color="primary"
    ...
    <div class="d-flex align-center ga-2">
      ...
      <v-menu v-model="menuEnd" :close-on-content-click="false" location="bottom start">
        ...
        <v-card min-width="auto">
          <v-date-picker v-model="internalEndDate" @update:model-value="onEndDateSelected" color="primary"
            hide-header></v-date-picker>
        </v-card>
      </v-menu>
    </div>

    <v-btn v-if="startDate || endDate" variant="text" density="compact" class="text-none px-2 text-caption"
      @click="clear">
      Καθαρισμός
    </v-btn>
  </div>
...
<style scoped>
/* Προσαρμογές style κουμπιών Vuetify για να ταιριάζουν με το shadcn/ui */
